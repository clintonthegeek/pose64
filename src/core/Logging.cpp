...
const uint32	kInvalidTimestamp		= (uint32)(uintptr_t) -1;
...
static string StubEmFrmGetTitle (const FormPtr frm)
{
	...
	{
		char	buffer[256];
		EmMem_strcpy (buffer, (emuptr)(uintptr_t) title);
...
static void StubEmPrintFormID (WinHandle winHandle, char* desc, char* eventText)
{
	...
	if (winHandle)
		{
		exitWinPtr = (emuptr)(uintptr_t) WinGetWindowPointer (winHandle);
		...
		while (winHandle)
			{
			winPtr = (emuptr)(uintptr_t) WinGetWindowPointer (winHandle);
...
static Bool PrvGetEventText(const EventType* eventP, char* eventText)
{
	...
	switch (eventP->eType)
	{
		...
		case tsmConfirmEvent:
			...
			EmMem_strncat(eventText, (emuptr)(uintptr_t)eventP->data.tsmConfirm.yomiText, kEventTextMaxLen - curLen);
