...
// 64-BIT FIX: Use C++ header for C++, C header for C (UAE m68k emulator)
#ifdef __cplusplus
#include <cstdint>
#else
#include <stdint.h>
...
// **CRITICAL 64-BIT FIX**: Changed from 'long' to 'int'
// On LP64 systems (64-bit Linux), sizeof(long)==8 but sizeof(int)==4
// The emulator assumes ILP32 semantics where int32/uint32 are exactly 4 bytes
	...
	// FIXED: was 'signed long' - breaks on 64-bit where long==8 bytes
