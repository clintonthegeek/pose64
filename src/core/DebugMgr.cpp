...
void Debug::Save (SessionFile& f)
{
	...
	{
		s << (emuptr)(uintptr_t) gDebuggerGlobals.bp[ii].addr;
...
void Debug::Load (SessionFile& f)
{
	...
	if (f.ReadDebugInfo (chunk))
	{
		...
		if (version >= 1)
		{
			...
			{
				emuptr temp;
				s >> temp; gDebuggerGlobals.bp[ii].addr = (MemPtr)(uintptr_t) temp;
				s >> gDebuggerGlobals.bp[ii].enabled;
				s >> gDebuggerGlobals.bp[ii].installed;
...
ErrCode Debug::ExitDebugger (void)
{
	...
	for (int ii = 0; ii < dbgTotalBreakpoints; ++ii)
	{
		...
		emuptr addr = (emuptr)(uintptr_t) gDebuggerGlobals.bp[ii].addr;
		if (!addr)
			continue;
...
void Debug::InstallInstructionBreaks (void)
{
	...
	for (int ii = 0; ii < dbgTotalBreakpoints; ++ii)
	{
		...
		{
			MetaMemory::MarkInstructionBreak ((emuptr)(uintptr_t) gDebuggerGlobals.bp[ii].addr);
		}
	}
}
...
void Debug::RemoveInstructionBreaks (void)
{
	...
	for (int ii = 0; ii < dbgTotalBreakpoints; ++ii)
	{
		if (gDebuggerGlobals.bp[ii].enabled)
		{
			MetaMemory::UnmarkInstructionBreak ((emuptr)(uintptr_t) gDebuggerGlobals.bp[ii].addr);
		}
	}
}
...
void Debug::SetBreakpoint (int index, emuptr addr, BreakpointCondition* c)
{
	...
	gDebuggerGlobals.bp[index].addr			= (MemPtr)(uintptr_t) addr;
...
{
	MemPtr	bpAddress = (MemPtr)(uintptr_t) m68k_getpc ();
