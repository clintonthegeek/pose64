...
// 64-BIT FIX: Use uint32 instead of unsigned long
uint32 gGremlinNext = 1;
...
static void FakeEventXY(Int16* x, Int16* y)
{
	...
	if (randN(20) == 1) {
		...
		emuptr			addr = ((emuptr)(uintptr_t) buttonListP) + randN(numButtons) * size;
...
Gremlins::New (const GremlinInfo& info)
{
	...
	if (info.fAppList.size () > 0)
	{
		...
		else
		{
			...
			emuptr	cmdPBP = (emuptr)(uintptr_t) ::MemPtrNew (sizeof (SysAppLaunchCmdOpenDBType));
			Errors::ThrowIfNULL ((void*)(uintptr_t) cmdPBP);

			// Fill it in
			::MemPtrSetOwner ((MemPtr)(uintptr_t) cmdPBP, 0);
			...
			// Switch now
			err = ::SysUIAppSwitch (cardNo, dbID, sysAppLaunchCmdOpenDB, (MemPtr)(uintptr_t) cmdPBP);
			Errors::ThrowIfPalmError (err);
		}
	}
