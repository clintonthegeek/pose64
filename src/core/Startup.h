...
typedef std::multimap<std::string, std::string>	PreferenceList;
...
class Startup
{
	public:
		...
		static std::string GetAutoRunApp			(void);
	...
	private:
		...
		static Bool				PrvConvertRAM			(const std::string& str, RAMSizeType& ramSize);
		static void				PrvParseFileList		(EmFileRefList& fileList, std::string option);
		...
		static std::string PrvTryGetApp			(const EmFileRefList& fileList);
