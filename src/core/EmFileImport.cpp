...
{
	ErrCode			err = errNone;
	EmStreamBlock	stream ((void*)(uintptr_t) data, size);
...
void EmFileImport::HomeBrewInstallStart (void)
{
	...
	if (!::PrvIsResources (hdr))
	{
		if (hdr.appInfoID)
		{
			...
			if (hdr.sortInfoID)
			{
				recSize = (UInt32)(uintptr_t) hdr.sortInfoID - (UInt32)(uintptr_t) hdr.appInfoID;
			}
			else if (hdr.recordList.numRecords > 0)
			{
				recSize = (UInt32)(uintptr_t) recordEntry.localChunkID - (UInt32)(uintptr_t) hdr.appInfoID;
			}
			else
			{
				recSize = fFileBufferSize - (UInt32)(uintptr_t) hdr.appInfoID;
			}
			...
			UInt8*	srcP = (UInt8*) hdr.GetPtr () + (UInt32)(uintptr_t) hdr.appInfoID;
...
void EmFileImport::HomeBrewInstallMiddle (void)
{
	...
	if (::PrvIsResources (hdr))
	{
		...
		{
			resSize = (UInt32)(uintptr_t) rsrcEntry[1].localChunkID;
			resSize -= (UInt32)(uintptr_t) rsrcEntry.localChunkID;
		}
		else
		{
			resSize = fFileBufferSize - (UInt32)(uintptr_t) rsrcEntry.localChunkID;
		}
		...
		srcP = (UInt8*) hdr.GetPtr () + (UInt32)(uintptr_t) rsrcEntry.localChunkID;
	...
	else
	{
		...
		if (fCurrentEntry < hdr.recordList.numRecords - 1)
		{
			recSize = (UInt32)(uintptr_t) recordEntry[1].localChunkID;
			recSize -= (UInt32)(uintptr_t) recordEntry.localChunkID;
		}
		else
		{
			recSize = fFileBufferSize - (UInt32)(uintptr_t) recordEntry.localChunkID;
		}
		...
		if (recSize)
		{
			...
			srcP = (UInt8*) hdr.GetPtr () + (UInt32)(uintptr_t) recordEntry.localChunkID;
...
static ErrCode PrvValidateDatabaseHeader(const EmAliasDatabaseHdrType<LAS>& hdr, UInt32 bufferSize)
{
	...
	char*	namePtr	= (char*)(uintptr_t) hdr.name.GetPtr ();
...
static ErrCode PrvValidateEntries(const EmAliasDatabaseHdrType<LAS>& hdr, UInt32 bufferSize)
{
	...
	for (UInt16 recordNumber = 0; recordNumber < hdr.recordList.numRecords; ++recordNumber)
	{
		...
		if (isResource)
		{
			...
			{
				itsSize = (UInt32)(uintptr_t) rsrcEntry[1].localChunkID;
				itsSize -= (UInt32)(uintptr_t) rsrcEntry.localChunkID;
			}
			else
				itsSize = bufferSize - (UInt32)(uintptr_t) rsrcEntry.localChunkID;
		...
		else
		{
			...
			if (recordNumber < hdr.recordList.numRecords - 1)
			{
				itsSize = (UInt32)(uintptr_t) recordEntry[1].localChunkID;
				itsSize -= (UInt32)(uintptr_t) recordEntry.localChunkID;
			}
			else
				itsSize = bufferSize - (UInt32)(uintptr_t) recordEntry.localChunkID;
...
void PrvSetExgMgr (void* mgr)
{
	...
	{
		emuptr	tblP = (emuptr)(uintptr_t) ::SysLibTblEntry (gHostExgLibRefNum);
		if (tblP)
		{
			EmAliasSysLibTblEntryType<PAS>	tbl (tblP);
			tbl.globalsP = (emuptr)(uintptr_t) mgr;
		}
	}
}
