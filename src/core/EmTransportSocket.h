...
{
	public:
								CTCPClientSocket		(EventCallback fn, std::string targetHost, int targetPort, EmTransportSocket* transport);
...
class EmTransportSocket : public EmTransport
{
	public:
		...
		struct ConfigSocket : public EmTransport::Config
		{
			...
			std::string					fTargetHost;
			std::string					fTargetPort;
		};

		typedef std::map<PortName, EmTransportSocket*>	OpenPortList;
