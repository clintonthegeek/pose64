...
static void PrvMyShlExportAsPilotFile(EmStreamFile& fh, UInt16 cardNo, const char* nameP)
{
	...
	try
	{
		...
		if (::PrvIsResources (attributes))
		{
			...
			strcpy ((char*)(uintptr_t) hdr.name.GetPtr (), nameP);
			...
			if (appInfoID && appInfoSize)
			{
				...
				srcP = (UInt32)(uintptr_t) ::MemHandleLock (appInfoH);
				EmMem_memcpy ((void*)(uintptr_t) outP.Get(), srcP, appInfoSize);
				::MemPtrUnlock ((MemPtr)(uintptr_t) srcP);
			...
			if (sortInfoID && sortInfoSize)
			{
				...
				srcP = (UInt32)(uintptr_t) ::MemHandleLock (sortInfoH);
				EmMem_memcpy ((void*)(uintptr_t) outP.Get(), srcP, sortInfoSize);
				::MemPtrUnlock ((MemPtr)(uintptr_t) srcP);
			...
			for (i=0; i < numRecords; i++)
			{
				...
				if (resChunkID)
				{
					...
					srcP = (UInt32)(uintptr_t) ::MemHandleLock (srcResH);
					EmMem_memcpy ((void*)(uintptr_t) outP.Get (), srcP, resSize);
					::MemPtrUnlock ((MemPtr)(uintptr_t) srcP);
		...
		else
		{
			...
			for (i=0; i<numRecords; i++)
			{
				...
				if (srcH)
				{
					...
					srcP = (UInt32)(uintptr_t) ::MemHandleLock (srcH);
					EmMem_memcpy ((void*)(uintptr_t) outP.Get(), srcP, recSize);
					::MemPtrUnlock ((MemPtr)(uintptr_t) srcP);
