...
/*
	...
	hierarchical information.  For instance:
		...
		Preference<std::string>				pref("GremlinInfo.fAppList[0].name");
*/
...
//		template <class T>	friend class Preference;
	public:
		bool					GetPref 			(const std::string& key, std::string& value);
		void					SetPref 			(const std::string& key, const std::string& value);
		void					DeletePref			(const std::string& key);
	...
	public:
		...
		std::string ExpandKey			(const std::string& name);
		std::string AppendName			(std::string key, const std::string& name);
	...
	public:
		...
		void					DoNotify			(const std::string& key);
	...
	protected:
		...
		typedef std::vector<PrefNotifyType>	PrefNotifyList;
