...
Bool EmCPU68K::ExecuteStoppedLoop (void)
{
	...
	// This is the ONE wake mechanism (recovery-plan 2.2/2.3) — do not add
	// a second.  PrvWakeUpCPU (the old main-thread EvtWakeup-via-stopper)
	// was deleted with this change.
	//
	// Only the `} while` fall-through reaches here; the CheckForBreak
	// `return true` above is a host-side suspend, not a guest wake, and
