...
char* ATrap::GetStackBase ()
{
	...
	// 64-BIT FIX: Use uintptr_t for pointer arithmetic instead of uint32
	uintptr_t stackBase = reinterpret_cast<uintptr_t>(fStack);

	stackBase += 3;
	stackBase &= ~3;

	return reinterpret_cast<char*>(stackBase);
}
