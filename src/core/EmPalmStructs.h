...
#define DEFINE_SCALAR_ALIAS(type, asType)											\
	...
	{																				\
		return EmAlias##type<A> ((ptr_type)(uintptr_t) (((char*)(uintptr_t) this->GetPtr ()) + index * this->GetSize ()));	\
	}																				\
																					\
	template <class A>																\
...
#define MAKE_ONE_SCALAR_ALIAS_ASSIGNMENT_OPERATOR(type, asType, rhs_type)			\
	...
	{																				\
		if (sizeof (asType) == 1)		A::PutByte (this->GetPtr (), (unsigned char) (asType)(uintptr_t) val);		\
		else if (sizeof (asType) == 2)	A::PutWord (this->GetPtr (), (unsigned short) (asType)(uintptr_t) val);	\
		else if (sizeof (asType) == 4)	A::PutLong (this->GetPtr (), (unsigned long) (asType)(uintptr_t) val);		\
		else BadSetter ();															\
																					\
		return *this;																\
	}
...
#define MAKE_ONE_SCALAR_PROXY_ASSIGNMENT_OPERATOR(type, asType, rhs_type)			\
	...
	{																				\
		if (sizeof (asType) == 1)		LAS::PutByte (this->GetPtr (), (unsigned char) (asType)(uintptr_t) val);	\
		else if (sizeof (asType) == 2)	LAS::PutWord (this->GetPtr (), (unsigned short) (asType)(uintptr_t) val);	\
		else if (sizeof (asType) == 4)	LAS::PutLong (this->GetPtr (), (unsigned long) (asType)(uintptr_t) val);	\
		else BadSetter ();															\
																					\
		return *this;																\
	}
...
#define MAKE_MEMBER_INITIALIZER_ALIAS(field_offset, field_type, field_name)			\
																					\
	, field_name((ptr_type)(uintptr_t) (((char*)(uintptr_t) fPtr) + field_offset))


#define MAKE_MEMBER_INITIALIZER_PROXY(field_offset, field_type, field_name)			\
																					\
	, field_name((ptr_type)(uintptr_t) (((char*)(uintptr_t) this->GetPtr ()) + field_offset))
