...
CallROMType MemMgrHeadpatch::MemChunkFree (void)
{
	...
	::PrvForgetChunk ((emuptr)(uintptr_t)(MemPtr) p);
	...
	::PrvRememberHeapAndPtr (heap, (emuptr)(uintptr_t)(MemPtr) p);
	...
	if (heap)
	{
		const EmPalmChunk*	chunk = heap->GetChunkBodyContaining ((emuptr)(uintptr_t)(MemPtr) p);
		if (chunk)
		{
...
CallROMType MemMgrHeadpatch::MemHandleFree (void)
{
	...
	::PrvForgetHandle ((emuptr)(uintptr_t) (MemHandle) h);

	// Remember what heap the chunk was in for later when
	// we need to resync with that heap.

	EmPalmHeap*	heap = const_cast<EmPalmHeap*>(EmPalmHeap::GetHeapByHdl (h));
	::PrvRememberHeapAndPtr (heap, (emuptr)(uintptr_t) (MemHandle) h);
	...
	if (h)
	{
		...
		emuptr	p = (emuptr)(uintptr_t) EmPalmHeap::DerefHandle (h);
...
CallROMType MemMgrHeadpatch::MemHandleUnlock (void)
{
	...
	if (MetaMemory::IsBitmapHandle (h))
	{
		...
		if (heap)
		{
			emuptr	p = (emuptr)(uintptr_t) EmPalmHeap::DerefHandle (h) - heap->ChunkHeaderSize ();

			EmPalmChunk	chunk (*heap, p);

			if (chunk.LockCount () == 1)
			{
				MetaMemory::UnregisterBitmapPointer ((MemPtr)(uintptr_t) p);
			}
		}
	}
...
void MemMgrTailpatch::MemChunkFree (void)
{
	...
	EmPalmHeap*	heap = ::PrvGetRememberedHeap ((emuptr)(uintptr_t)(MemPtr) p);
...

	{
		...
		if (w == (emuptr)(uintptr_t)(MemPtr) p)
		{
...
//	CALLED_GET_PARAM_VAL (UInt32, size);
	...
	if (gPrefs->FillNewBlocks ())
	{
		if (c)
		{
			...
			{
				emuptr	p = (emuptr)(uintptr_t) ::MemHandleLock ((MemHandle) c);
				if (p)
				{
					{
						CEnableFullAccess	munge;
	...
	EmPalmChunkList	delta;
	EmPalmHeap::MemChunkNew (heapID, (MemPtr)(uintptr_t) result, attr, &delta);
	MetaMemory::Resync (delta);
...
#define TRACK_BOOT_ALLOCATION 0
	...
	const EmPalmHeap*	heap = EmPalmHeap::GetHeapByPtr ((MemPtr)(uintptr_t) result);
	if (heap)
	{
	...
	{
		MemHandle	h = EmPalmHeap::RecoverHandle ((MemPtr)(uintptr_t) result);
		::PrvRememberHandle ((emuptr)(uintptr_t) h);
	}
	else
	{
...
void MemMgrTailpatch::MemHandleFree (void)
{
	...
	EmPalmHeap*	heap = ::PrvGetRememberedHeap ((emuptr)(uintptr_t) (MemHandle) h);
...
void MemMgrTailpatch::MemHandleNew (void)
{
	...
	if (result)
	{
		...
		{
			emuptr	p = (emuptr)(uintptr_t) ::MemHandleLock ((MemHandle) result);
			if (p)
			{
				{
					CEnableFullAccess	munge;
	...
	EmPalmChunkList	delta;
	EmPalmHeap::MemHandleNew ((MemHandle)(uintptr_t) result, &delta);
	MetaMemory::Resync (delta);
	...
	const EmPalmHeap*	heap = EmPalmHeap::GetHeapByPtr ((MemPtr)(uintptr_t) c);
	if (heap)
	{
...
void MemMgrTailpatch::MemHandleLock (void)
{
	...
	{
		GET_RESULT_PTR ();
		MetaMemory::RegisterBitmapPointer ((MemPtr)(uintptr_t) result);
	}
...
void MemMgrTailpatch::MemHandleResize (void)
{
	...
	if (h)
	{
		...
		{
			emuptr	p = (emuptr)(uintptr_t) ::MemHandleLock ((MemHandle) h);
			if (p)
			{
				{
					CEnableFullAccess	munge;
...
void MemMgrTailpatch::MemLocalIDToLockedPtr (void)
{
	...
	EmPalmChunkList	delta;
	EmPalmHeap::MemLocalIDToLockedPtr ((MemPtr)(uintptr_t) result, &delta);
	MetaMemory::Resync (delta);
...
void MemMgrTailpatch::MemPtrNew (void)
{
	...
	EmPalmChunkList	delta;
	EmPalmHeap::MemPtrNew ((MemPtr)(uintptr_t) result, &delta);
	MetaMemory::Resync (delta);
...
void PrvRememberChunk (emuptr p)
{
	...
	MemHandle	h = EmPalmHeap::RecoverHandle ((MemPtr)(uintptr_t) p);
	...
	while (iter != EmPatchState::fgData.fTrackedChunks.end ())
	{
		...
		if (iter->isHandle)
		{
			emuptr	hDereffed = (emuptr)(uintptr_t) EmPalmHeap::DerefHandle ((MemHandle)(uintptr_t) iter->ptr);
			if (hDereffed == p)
			{
				EmAssert (false);
			}
...
void PrvRememberHandle (emuptr h)
{
	...
	const EmPalmHeap*	heap = EmPalmHeap::GetHeapByHdl ((MemHandle)(uintptr_t) h);
...
void PrvForgetChunk (emuptr p)
{
	...
	if (h)
	{
		::PrvForgetHandle ((emuptr)(uintptr_t) h);
		return;
	}
...
void PrvForgetAll (UInt16 heapID, UInt16 ownerID)
{
	...
	while (iter != EmPatchState::fgData.fTrackedChunks.end ())
	{
		...
		if (iter->isHandle)
		{
			heap = EmPalmHeap::GetHeapByHdl ((MemHandle)(uintptr_t) iter->ptr);

			if (heap && heap->HeapID () == heapID)
			{
				chunk = heap->GetChunkReferencedBy ((MemHandle)(uintptr_t) iter->ptr);
			}
		}
		else
		{
...
void PrvReportLeakedChunk (const EmTrackedChunk& tracked, const EmPalmChunk& chunk)
{
	...
	EmMem_memcpy ((void*)(uintptr_t) buffer, chunk.BodyStart (), len);
