...
CallROMType SysHeadpatch::SysAppExit (void)
{
	...
	{
		EmuAppInfo	appInfo;
		EmPatchState::CollectCurrentAppInfo ((emuptr)(uintptr_t) appInfoP, appInfo);
...
CallROMType SysHeadpatch::SysUIAppSwitch (void)
{
	...
	//	and free the chunk itself  when the app quits.
	{
		...
		if (nextUIAppCmdPBP)
		{
			MemPtrFree((MemPtr)(uintptr_t) nextUIAppCmdPBP);
	...
	//	CEnableFullAccess	munge;	// Remove blocks on memory access.
	//	EmLowMem_SetGlobal (nextUIAppCmdPBP, (MemPtr)(uintptr_t) cmdPBP);
...
#define bitmapRsc 	'Tbmp'
	...
	{
		GET_RESULT_PTR ();
		MetaMemory::RegisterBitmapHandle ((MemHandle)(uintptr_t) result);
	}
}
...
void SysTailpatch::ExgReceive (void)
{
	...
	{
		LogAppendData ((void*)(uintptr_t) bufP, bufLen, "ExgMgr: received %ld bytes, err = 0x%04X.", result, *errP);
	}
}
...
void SysTailpatch::ExgSend (void)
{
	...
	{
		LogAppendData ((void*)(uintptr_t) bufP, bufLen, "ExgMgr: sent %ld bytes, err = 0x%04X.", result, *errP);
	}
}
...
void PrvCopyPalmClipboardToHost (void)
{
	...
	if (length > 0)
	{
		...
		if (dataPtr)
		{
			...
			if (dataCopy)
			{
				EmMem_memcpy (dataCopy, (emuptr)(uintptr_t) dataPtr, length);

				ByteList	palmChars;
				ByteList	hostChars;
				char*		charData = (char*)(uintptr_t) dataCopy;
