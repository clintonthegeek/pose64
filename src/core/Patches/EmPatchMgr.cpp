...
void EmPatchMgr::HandleInstructionBreak (void)
{
	...
	emuptr breakPC = gCPU->GetPC ();
	TailpatchProc	tp = RecoverFromTailpatch (breakPC);
...
Err EmPatchMgr::SwitchToApp (UInt16 cardNo, LocalID dbID)
{
	...
	else
	{
		...
		err = ::SysUIAppSwitch (appCardNo, appDbID, sysAppLaunchCmdOpenDB, (MemPtr)(uintptr_t) cmdPBP);
		if (err)
			return err;
	}
