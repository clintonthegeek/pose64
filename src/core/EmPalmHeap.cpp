...
{
	return GetHeapByPtr ((MemPtr)(uintptr_t) p);
}
...
const EmPalmHeap* EmPalmHeap::GetHeapByPtr (MemPtr p)
{
	...
	while (iter != fgHeapList.end ())
	{
		if (iter->Contains ((emuptr)(uintptr_t) p))
		{
			return &*iter;
		}
...
MemPtr EmPalmHeap::DerefHandle (MemHandle h)
{
	...
	emuptr	pp = (emuptr)(uintptr_t) memHandleUnProtect(h);
	return (MemPtr)(uintptr_t) EmMemGet32 (pp);
}
...
MemHandle EmPalmHeap::RecoverHandle (MemPtr p)
{
	...
	if (p)
	{
		...
		if (heap)
		{
			const EmPalmChunk*	chunk = heap->GetChunkBodyContaining ((emuptr)(uintptr_t) p);

			if (chunk && !chunk->Free () && chunk->HOffset ())
			{
				h = (MemHandle)(uintptr_t) (((emuptr)(uintptr_t) p) - chunk->HOffset () * 2 - chunk->HeaderSize ());

#if _DEBUG
				CEnableFullAccess	munge;	// Remove blocks on memory access.
				EmAssert (EmMemGet32 ((emuptr)(uintptr_t) h) == (emuptr)(uintptr_t) p);
#endif
...
void EmPalmHeap::GetHeapHeaderInfo (UInt16 heapID)
{
	...
	if (heapHdr)
		this->GetHeapHeaderInfo ((emuptr)(uintptr_t) heapHdr);
}
...
void EmPalmHeap::ResyncPtr (MemPtr p, EmPalmChunkList* delta)
{
	...
	emuptr	chunkStart = ((emuptr)(uintptr_t) p) - fChunkHdrSize;
