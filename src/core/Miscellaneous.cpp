	...
	Bool	inClearBytes)			// Whether to clear all bytes to zero
{
	...
	if (inClearBytes)
		mPtr = (char*)(uintptr_t) Platform::AllocateMemoryClear (inSize);
	else
		mPtr = (char*)(uintptr_t) Platform::AllocateMemory (inSize);
}
									   ...
									   RectangleType bounds)
{
	...
	{
		emuptr	listPtr = (emuptr)(uintptr_t) FrmGetObjectPtr (frm, objIndex);
		Int16	numItems = ::LstGetNumberOfItems ((ListType*)(uintptr_t)listPtr);
...
static Bool PrvFormObjectIsUsable (FormPtr frmP, uint16 index, FormObjectKind kind)
{
	...
	emuptr objP = (emuptr)(uintptr_t)::FrmGetObjectPtr (frmP, index);
	...
	switch (kind)
	{
		...
		{
			FieldAttrType attr;
			::FldGetAttributes ((FieldType*)(uintptr_t)objP, &attr);
...
static Err AppGetExtraInfo (DatabaseInfo* infoP)
{
	...
	if (infoP->dbAttrs & dmHdrAttrResDB)
	{
		...
		if (strH != NULL)
		{
			emuptr strP = (emuptr)(uintptr_t) MemHandleLock (strH);
	...
	else
	{
		...
		if (!err && appInfoID)
		{
			...
			//	out of that.
			specialInfoP = (emuptr)(uintptr_t) appInfoP;
	...
	for (int kk = 0; kk < 3 && !perfect; ++kk)
	{
		...
		if (dbP)
		{
			...
			if (resourceH)
			{
				...
				perfect = (EmMem_memcmp ((void*)(uintptr_t) perfect_pattern, (emuptr)(uintptr_t) resP, 8) == 0);
...
void SetHotSyncUserName (const char* userNameP)
{
	...
	DlpTinyArgWrapperType*		reqWrapper	= (DlpTinyArgWrapperType*) (((char*)(uintptr_t) reqHdr) + sizeof(DlpReqHeaderType));
	DlpWriteUserInfoReqHdrType* reqArgHdr	= (DlpWriteUserInfoReqHdrType*) (((char*)(uintptr_t) reqWrapper) + sizeof(DlpTinyArgWrapperType));
	char*						reqName		= ((char*)(uintptr_t) reqArgHdr) + sizeof (DlpWriteUserInfoReqHdrType);
...
void RunLengthEncode (void** srcPP, void** dstPP, int32 srcBytes, int32 dstBytes)
{
	...
	uint8* srcP = (uint8*)(uintptr_t) *srcPP;
	uint8* dstP = (uint8*)(uintptr_t) *dstPP;
	uint8* opP = NULL;
	...
	*srcPP = (void*)(uintptr_t) srcP;
	*dstPP = (void*)(uintptr_t) dstP;
}
...
void GzipEncode (void** srcPP, void** dstPP, int32 srcBytes, int32 dstBytes)
{
	...
	// Raw deflate (no zlib/gzip header) to match original POSE format
	deflateInit2 (&strm, Z_BEST_SPEED, Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY);

	strm.next_in   = (Bytef*) *srcPP;
	strm.avail_in  = srcBytes;
	strm.next_out  = (Bytef*) *dstPP;
	strm.avail_out = dstBytes;

	::deflate (&strm, Z_FINISH);
	deflateEnd (&strm);

	*srcPP = (char*)(uintptr_t) *srcPP + strm.total_in;
	*dstPP = (char*)(uintptr_t) *dstPP + strm.total_out;
}
...
void GzipDecode (void** srcPP, void** dstPP, int32 srcBytes, int32 dstBytes)
{
	...
	// Raw inflate (no zlib/gzip header) to match original POSE format
