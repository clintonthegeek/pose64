	...
	if ((currentFormID != fgPreviousFormID) && currentFormID)
	{
		...
		if (currentFormID)
		{
			...
			if (fgEventInfo.size () > 0)
			{
				...
				EmAliasFormType<PAS> form ((emuptr)(uintptr_t) frmP);

				// Get the new form's title.

				emuptr emuTitle = (emuptr)(uintptr_t)::FrmGetTitle (frmP);
...
static Boolean PrvPointInTableItem (TableType* tableP, Coord x, Coord y, Int16& rrow, Int16& rcol)
{
	...
	EmAliasTableType<PAS> table ((emuptr)(uintptr_t)tableP);
...
void EmEventOutput::GetPenEventInfo (EmEventInfo& eventInfo)
{
	...
	if (iter != objectIndices.end ())
	{
		...
		switch (eventInfo.objKind)
		{
			...
			case frmControlObj:
			{
				emuptr	ctlP = (emuptr)(uintptr_t) ::FrmGetObjectPtr (frmP, *iter);
...
//				event.text = "titletext";
	...
	else 
	{
		...
		EmAliasPenBtnInfoType<PAS>	buttonList ((emuptr)(uintptr_t) buttonListP);
			...
			case kRecordedPenEvent:
			{
				switch (awaitingEvent.objKind)
				{
					case frmFieldObj:
					{
						...
						if (fldP)
						{
							emuptr txtP = (emuptr)(uintptr_t) ::FldGetTextPtr (fldP);
					...
					case frmListObj:
					{
						...
						if (lstP)
						{
							...
							if (currentItem != noListSelection)
							{
								...
								emuptr textP = (emuptr)(uintptr_t) ::LstGetSelectionText (lstP, currentItem);
