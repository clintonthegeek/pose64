...
class EmTransportDescriptor
{
	public:
								...
								EmTransportDescriptor	(EmTransportType, const std::string&);
								EmTransportDescriptor	(const std::string&);
		...
		std::string GetMenuName				(void) const;	// For menus (e.g., "COM1", "TCP/IP")
		std::string GetDescriptor			(void) const;	// Full text (e.g., "serial:COM1")
		std::string GetScheme				(void) const;	// First part of text (e.g., "serial", "socket")
		std::string GetSchemeSpecific		(void) const;	// Second part of text (e.g., "127.0.0.1:5001")
		EmTransportType			GetType					(void) const;	// Scheme in numeric form

		static std::string GetSchemePrefix			(EmTransportType);	// Convert numeric format to scheme prefix
...
typedef std::vector<EmTransportDescriptor>	EmTransportDescriptorList;
...
class EmTransport
{
	...
	public:
		...
		virtual std::string GetSpecificName		(void) = 0;
...
class EmTransportNull : public EmTransport
{
	...
	public:
		...
		virtual std::string GetSpecificName			(void);
