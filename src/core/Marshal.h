...
#define PUT_RESULT_VAL(type, val)							\
	Marshal::PutReturnVal (sub, (type)(uintptr_t) val);
...
#define CALLER_PUT_PARAM_VAL(type, name)					\
	PushParamVal<type>		_##name (sub, #name, (type)(uintptr_t) name);		\
...
#define DECLARE_SCALAR_MARSHALLER(type, asType, num_bits)				\
		...
		inline static void PutReturnVal (EmSubroutine& sub, type v)		\
			{ asType _t = (asType)(uintptr_t) v; sub.SetReturnVal (_t); }	\
																		\
		...
		// 64-BIT FIX: On 64-bit platforms, long is 8 bytes but we need 32-bit compatibility
		// These marshallers cast long to int32 to maintain the emulator's 32-bit semantics
		DECLARE_SCALAR_MARSHALLER (long, int32, 32)
		DECLARE_SCALAR_MARSHALLER (unsigned long, uint32, 32)
...
/* ===========================================================================
	...
	std::string by calling EmMem_strlen).  The string is modifiable in-place, but
