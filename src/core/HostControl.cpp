...
static void _HostFPrintF (void)
{
	...
	int 	result = x_vfprintf (fh, fmt, *(va_list*)(void*) &stackData[0]);
...
static void _HostTmpNam (void)
{
	...
	if (fd > 0 && !close (fd))
	{
		...
		if (nameP != EmMemNULL)
		{
			strcpy ((char*)(uintptr_t) nameP, temp_buff);
		}
	}
...
static void _HostReadDir(void)
{
	...
	MyDIR*	dir = (MyDIR*)(uintptr_t) dirP;
...
static void _HostGetPreference (void)
{
	...
	if (pref.Loaded ())
	{
		EmMem_strcpy ((emuptr)(uintptr_t) value, pref->c_str ());
...
static void _HostSlotMax (void)
{
	...
	Preference<SlotInfoList>	slotPrefs (kPrefKeySlotList);

	SlotInfoList::const_iterator	iter = (*slotPrefs).begin ();
	while (iter != (*slotPrefs).end ())
	{
...
void PrvPushLong (EmSubroutine& sub, ByteList& stackData)
{
	...
	ByteList::size_type	oldSize = stackData.size ();
	stackData.insert (stackData.end (), sizeof (uint32), 0);	// Make space for a "long int"

	*(uint32*) &stackData[oldSize] = value;
}
...
{
	if ((HostFILEType*)(uintptr_t) f == hostLogFile)
	{
		return hostLogFILE;
	}
