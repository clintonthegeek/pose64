...
class EmHostTransportSerial
{
	public:
		...
		// PTY support
		ErrCode					OpenPtyPort			(const std::string& portName);
	...
	public:
		...
		int						fTimeToQuit;
		int						fPtyMaster;		// nonzero when using PTY mode
		std::string				fPtySlaveName;	// slave path (e.g. /dev/pts/7)
